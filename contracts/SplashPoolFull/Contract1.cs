...
namespace SplashPoolFull
{
    public class Contract1 : SmartContract
    {
        ...
        //TestNet
        //[AppCall("c186bcb4dc6db8e08be09191c6173456144c4b8d")] // ScriptHash
        //public static extern int MCTContract(string operation, object[] args);

        // Events
        [DisplayName("created")]
        public static event Action<byte[], byte[], byte[], BigInteger> Created; // (address, offerHash, offerAssetID, offerAmount, wantAssetID, wantAmount)

        [DisplayName("filled")]
        public static event Action<byte[], byte[], BigInteger, byte[], BigInteger> Filled; // (address, offerHash, fillAmount, offerAssetID, offerAmount, wantAssetID, wantAmount)
        ...
        private struct Pool
        {
            ...
            public byte[] Nonce;
            public BigInteger CurrentEpoch;
        }
        ...
        public static object Main(string operation, params object[] args)
        {
            ...
            else if (Runtime.Trigger == TriggerType.Application)
            {
                ...
                //Where the user deposits their coins for contribution
                {
                    ...
                    var contribution = NewContribution((byte[])args[0], (byte[])args[1], (byte[])args[2], (byte[])args[3], (byte[])args[4], (byte[])args[5], (BigInteger)args[6]);
                ...
                if (operation == "completePool")
                {
                    ...
                    //var completePool = ComptePoolOperation((byte[])args[0], (byte[])args[1], (byte[])args[2], (byte[])args[3], (byte[])args[4], (byte[])args[5], (byte[])args[6]);
                ...
                if (operation == "createUpdatePool")
                {
                    ...
                    return CreateUpdatePool((byte[])args[0], (byte[])args[1], (byte[])args[2], (byte[])args[3], (BigInteger)args[4], (BigInteger)args[5], (BigInteger)args[6], (BigInteger)args[7], (BigInteger)args[8], (byte[])args[9], (byte[])args[10], (BigInteger)args[11]);
                }
        ...
        private static Pool[] GetContributions(byte[] poolID, byte[] offset) // offerAssetID.Concat(wantAssetID)
        {
            ...
            var it = Storage.Find(Context(), poolID);
        ...
        private static bool CreateUpdatePool(byte[] makerAddress, byte[] poolID, byte[] poolCategory, byte[] poolAssetID, BigInteger maxPool, BigInteger minDeposit, BigInteger amount, BigInteger startTime, BigInteger endTime, byte[] makerCommand, byte[] nonce, BigInteger currentEpoch)
        {
            Pool pool = new Pool();
            pool.MakerAddress = makerAddress;
            pool.PoolID = poolID;
            pool.PoolCategory = poolCategory;
            pool.PoolAssetID = poolAssetID;
            pool.MaxPool = maxPool;
            pool.MinDeposit = minDeposit;
            pool.Amount = amount;
            pool.StartTime = startTime;
            pool.EndTime = endTime;
            pool.MakerCommand = makerCommand;
            pool.CurrentSize = 0;
            pool.Nonce = nonce;
        ...
        private static bool MakeContribution(Pool pool)
        {
            ...
            if (!(pool.Amount > pool.MinDeposit)) return false;

            //Check that Amount !> Max
            if (((pool.Amount + pool.CurrentSize)) < pool.MaxPool) return false;
        ...
        private static bool CompletePool(byte[] operatorAddress, byte[] poolID, byte[] poolHash, BigInteger amountToFill, bool useNativeTokens)
        {
            ...
            Pool pool = GetPool(poolID, poolHash);
        ...
        */
        ...
        private static void StorePool(byte[] poolID, byte[] poolHash, Pool pool)
        {
            ...
            else
            {
                ...
                Storage.Put(Context(), poolID, poolData);
            }
        }
        ...
        private static byte[] PoolID(Pool o) => o.PoolID;
